...
def dehaze(path, output = None):
    ...
    trans_guided = np.zeros((h, w), dtype=np.float)#注销以切换方式
    trans_guided = cv2.max(trans_guided, 0.3)

    result = np.empty_like(img)
    for i in range(3):
        # result[:, :, i] = (img[:, :, i] - atom) / trans_guided + atom
        result[:, :, i] = img[:, :, i]*trans_guided + atom*(1-trans_guided)
...
if __name__ == '__main__':
    img_dir = '/work/meteorology/data/WRGC/classify/预置位4'
    # img_dir = '/data_1/data/dehaze/synthetic/clear/clear_images'
    save_dir = '/work/meteorology/data/WRGC/classify/预置位4_hazy_1'
    for item in os.listdir(img_dir):
        img_path = os.path.join(img_dir, item)
        save_path = os.path.join(save_dir, item)
        dehaze(img_path, save_path)

    # if args.input is None:
    #     dehaze('/data_1/data/dehaze/real-world/NW_Google_837.jpeg')
    # else:
