...
def main():
    docx_root = '/work/meteorology/2018_docx'
    txt_root = '/work/meteorology/2018_txt'
    for i, folder in enumerate(os.listdir(docx_root)):
        print("========== Deal with the folder %s[%s th] ==========\n" %(folder, i+1))
        docx_dir = os.path.join(docx_root, folder)
        txt_dir = os.path.join(txt_root, folder)
        BatchDoc2Txt(docx_dir, txt_dir)
