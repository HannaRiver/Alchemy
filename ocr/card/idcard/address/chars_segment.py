...
def get_chars_bnbox(image_file, detection, args, save_path='', showkey=False):
    ...
    if len(results) == 0:
        if showkey:
            logging.info("模型认为该图不存在可分割字符 --> %s" %(image_file))
    ...
    for j, result in enumerate(results):
        show_img = img.copy()
        label = result[4]
        height, width = args.image_resize

        xmin, xmax = [int(round(result[i] * width)) for i in [0, 2]]
        ymin, ymax = [int(round(result[i] * height)) for i in [1, 3]]
        xmin, ymin = map4undeform_center_resize([xmin, ymin], args.image_resize, img.shape[: 2])
        xmax, ymax = map4undeform_center_resize([xmax, ymax], args.image_resize, img.shape[: 2])
...
def get_chars_bnboxs(args, test_root_dir='', _IS_SAVE_=False, showkey=False, version='char_seg_v000'):
    ...
    return chars_bnbox_list, img_path_list
