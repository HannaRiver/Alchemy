...
sys.path.append(os.path.join(os.path.dirname(sys.argv[0]), '..', 'address'))
...
def chars_cls_args():
    ...
    parser.add_argument('--resize_type', default='adaptiveBinary')
    parser.add_argument('')
    ...
    parser.add_argument('--model_def',
                        default='/work/ocr/card/model/owner/V0.1.0/deploy_train_zifu.prototxt')
    parser.add_argument('--image_resize', default=[24, 24], type=int)
    parser.add_argument('--model_weights',
                        default='/work/ocr/card/model/owner/V0.1.0/owner-clc_iter_209000.caffemodel')
...
if __name__ == '__main__':
    results, img_path_list= address_recognise('/work/ocr/card/vehicle_license/data/test/cut/3Owner/rename', _IS_DEBUG_CLS_=False, showkey=False)
    cnt, cnt_r = 0, 0
    for i, result in enumerate(results):
        cnt += 1
        model_result = ''.join(j for j in [k for k in result if k != 'none'])
        print('img_name: %s\t rec result: %s' %(img_path_list[i], model_result))
        if img_path_list[i][: -4] == model_result:
            cnt_r += 1
    print(1.0*cnt_r/cnt)
