...
def shred_det_args():
    ...
    parser.add_argument('--labelmap_file', default=cfg.CRCB_MODEL.SHRED_DET_LABELMAP)
    parser.add_argument('--mean_value', default=cfg.CRCB_MODEL.SHRED_DET_MEANVALUE)
    parser.add_argument('--input_scale', default=cfg.CRCB_MODEL.SHRED_DET_SCALE)
    parser.add_argument('--resize_type', default=cfg.CRCB_MODEL.SHRED_DET_RESIZETYPE)
    parser.add_argument('--model_def', default=cfg.CRCB_MODEL.SHRED_DET_MODELDEF)
    parser.add_argument('--image_resize', default=cfg.CRCB_MODEL.SHRED_DET_RESIZESIZE, type=int)
    parser.add_argument('--model_weights', default=cfg.CRCB_MODEL.SHRED_DET_MODELWEIGHT)
...
def shred_rec_args():
    ...
    parser.add_argument('--gpu_id', type=int, default=0, help='gpu id')
    parser.add_argument('--model_def', default=cfg.CRCB_MODEL.SHRED_REC_MODELDEF)
    parser.add_argument('--image_resize', default=cfg.CRCB_MODEL.SHRED_REC_RESIZESIZE, type=int)
    parser.add_argument('--time_step', default=cfg.CRCB_MODEL.SHRED_REC_TIMESTEP)
    parser.add_argument('--scale', default=cfg.CRCB_MODEL.SHRED_REC_SCALE)
    ...
    parser.add_argument('--model_weights', default=cfg.CRCB_MODEL.SHRED_REC_MODELWEIGHT)
    return parser.parse_args()
...
def check_label_args():
    ...
    parser.add_argument('--version', default=cfg.CRCB_DATA.DET2REC_VERSION, help='端对端版本')
    return parser.parse_args()

def det2Rec4Net(img_dir, rec_item, _IS_SAVE_DET_=False, _IS_DEBUG_CLS_=False, _IS_DEBUG_REC_=False, showkey=False, version='char_seg_v000'):
    '''
    模型从定位到识别结果组合
    ...
    for i, item in enumerate(shred_bnbox_list):
        ...
        for obj in item:
            ...
            pre_result, _, _, = refuse2plate(chars_img, recognition, rec_item, _IS_DEBUG_REC_, label)
...
def det2Rec4LabelData(img_dir, save_right_dir, save_error_dir, rec_item='CA', label_indx=-1, version='v0.0.0'):
    ...
    net_result = det2Rec4Net(img_dir, rec_item, False, False, False, False, version)
