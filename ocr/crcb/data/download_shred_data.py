...
def downloadShredData(date):
    data_cnt = 0 #统计所有大类的数量
    args = download_args()
    save_dir = os.path.join(args.save_dir, args.cls_type) # 保存下载的目录路径 xxxx/CA/20190902/
    ...
    for item in need_item:
        ...
        if not os.path.exists(copy_path):
            print("Path:%s not data!" %(copy_path))
            continue
    ...
    print("========== Data: %s Type: %s IsHd: %s Size: %s ==========" %(date, args.cls_type, is_hd, data_cnt))
