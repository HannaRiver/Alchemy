...
def det_labeld_args():
    ...
    parser.add_argument('--img_dir', default=cfg.CRCB_DATA.DETLABELED_DIR, help='需要处理数据的地址')
    parser.add_argument('--save_dir', default=cfg.CRCB_DATA.CHECKDATA_DIR_PRINT, help='需要处理数据的保存地址')
    parser.add_argument('--label_indx', type=int, default=-1, help='碎片数据gt的位置')
    parser.add_argument('--key_date', default=cfg.CRCB_DATA.DETLABELED_KEYDATE, type=str, help='需要处理的日期批次号')
