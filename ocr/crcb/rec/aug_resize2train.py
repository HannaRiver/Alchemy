...
def augResize_args():
    ...
    parser.add_argument('--save_test_dir', default=cfg.CRCB_DATA.TEST_DIR, help='测试集原图保存地址')

    # 拓增所需要的参数
    parser.add_argument('--download_dir', default=cfg.CRCB_DATA.DOWNLOAD_DIR_PRINT, help='原始完整数据所在的根目录')
    parser.add_argument('--image_resize', default=cfg.CRCB_MODEL.SHRED_REC_RESIZESIZE, type=int)
    parser.add_argument('--mean_value', default=cfg.CRCB_MODEL.SHRED_REC_MEANVALUE)
    parser.add_argument('--resize_type', default=cfg.CRCB_MODEL.SHRED_REC_RESIZETYPE)
...
def AugResize2Train(check_state_dir, image_resize, mean_value, save_train_dir, save_test_dir, download_dir, save_bak_dir='', save_test_org_dir='', ratio=0.2, aug_times=5):
    ...
    for i, img_name in enumerate(img_list):
        ...
        if i < len(img_list)*ratio:
            shutil.move(org_img_path, os.path.join(save_test_org_dir, label + '_' + org_img_name))
            continue
        # ====== Trian Part Aug =====

        roi_xmin, roi_xmax, roi_ymin, roi_ymax = [int(i) for i in img_name_infos[-7: -3]]
        roi_x_o, roi_y_o, roi_w_o, roi_h_o = roi_xmin, roi_ymin, roi_xmax - roi_xmin, roi_ymax - roi_ymin
...
def BatchAugResize2Train(check_state_root, image_resize, mean_value, save_train_dir, save_test_dir, download_root, save_bak_dir, save_test_org_dir):
    ...
    if not os.path.exists(save_test_org_dir):
        os.makedirs(save_test_org_dir)
    for item in os.listdir(check_state_root):
        ...
        AugResize2Train(check_state_dir, image_resize, mean_value, save_train_dir, save_test_dir, download_dir, save_bak_dir, save_test_org_dir)
...
def main():
    ...
    for date in dates:
        ...
        save_test_org_dir = os.path.join(args.save_test_dir, args.item_type, date)
        download_root = os.path.join(args.download_dir, args.item_type, date)
        logging.info("Date: %s ItemType: %s " %(date, rec_item))
        logging.info("Download root: %s" %(download_root))
        logging.info("For TrainSet Path: %s" %(save_train_dir))
        logging.info("Save Org TestSet Path: %s" %(save_test_org_dir))

        BatchAugResize2Train(check_state_root, args.image_resize, args.mean_value, save_train_dir, save_test_dir, download_root, save_bak_dir, save_test_org_dir)
